...
# ------------------
# ✅ 전처리: 이미지 선명도 향상을 위한 전처리 함수 (OpenCV 사용)
# ------------------
def preprocess_for_ocr(image_file):
    # 이미지 파일의 버퍼를 NumPy 배열로 변환
    file_bytes = np.asarray(bytearray(image_file.read()), dtype=np.uint8)
    img = cv2.imdecode(file_bytes, cv2.IMREAD_COLOR)
    # 흑백 변환
    gray = cv2.cvtColor(img, cv2.COLOR_BGR2GRAY)
    # 이진화: 임계값 150을 기준으로 흰색, 검은색 구분
    _, thresh = cv2.threshold(gray, 150, 255, cv2.THRESH_BINARY)
    return thresh

# ------------------
# ✅ OCR: 이미지에서 텍스트 추출 (전처리 포함)
# ------------------
...
# ------------------
# ✅ OCR 텍스트 파싱: 복잡한 결과에서 아파트명, 시세, 평수, 층수, 방향 추출
# ------------------
def parse_ocr_text(ocr_text):
    """
    예시 OCR 결과: 
    "힐스테이트태전 1518동 매매 5억 9,800, 25.4py, 13/18층 남동향"
    추출:
      - 아파트명: "힐스테이트태전 1518동"
      - 시세: 5억 9,800 → 59800 (만원)
      - 평수: 25.4 (py)
      - 층수: "13/18"
      - 방향: "남동향"
    """
    # 1. 아파트명 추출: "매매" 앞까지를 이름으로 간주
    name_pattern = r"^(.*?)\s+매매"
    nm = re.search(name_pattern, ocr_text)
    apt_name = nm.group(1).strip() if nm else None

    # 2. 시세 추출: "매매" 이후 "억"과 그 다음 숫자
    ...
    if match_price:
        ...
        price_val = int(main_val * 10000 + sub_val)  # 1억 = 10000만원
...
if uploaded_image:
    # 이미지 파일을 재설정하기 위해 buffer 복사
    image_bytes = uploaded_image.read()
    # OCR 전처리 및 추출 (버퍼 사용 후 재설정 필요)
